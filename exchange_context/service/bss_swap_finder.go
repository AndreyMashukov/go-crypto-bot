...
type BSSSwapFinder struct {
	ExchangeRepository ExchangeRepository.SwapPairRepositoryInterface
	Formatter          *Formatter
}

func (s *BSSSwapFinder) Find(quote string) model.BBSArbitrageChain {
	...
	for _, option0 := range options0 {
		...
		buy0 := model.SwapTransition{
			...
			Type:          model.SwapTransitionTypeBuySellSell,
		...
		options1 := s.ExchangeRepository.GetSwapPairsByBaseAsset(buy0.BaseAsset, "USDT")
		for _, option1 := range options1 {
			...
			options2 := s.ExchangeRepository.GetSwapPairsByBaseAsset(option1.QuoteAsset, "")
