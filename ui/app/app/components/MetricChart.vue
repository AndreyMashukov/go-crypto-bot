...
// MetricChart — Prometheus PromQL → eCharts line chart. Ported from
// Phase G; rewired to use Pinia $api so the Bearer-token-less /api/*
// proxy chain (Nuxt → ui-server → Prometheus) handles the request.
...
interface SeriesPoint { 0: number; 1: string }
interface Series { label: string; points: SeriesPoint[] }
interface PrometheusRangeResponse { series: Series[] }
...
async function fetchSeries() {
  error.value = null
  try {
    const response = await $api<PrometheusRangeResponse>('/prometheus/range', {
      query: {
        query: props.query,
